...
<style>
    ...
    .result-label { font-size: 2rem; font-weight: 900; margin: 0; }
...
input_data = pd.DataFrame([[
    age_at_diagnosis, tumor_size, float(neoplasm_histologic_grade),
    float(lymph_nodes_examined_positive), float(mutation_count), nottingham_prognostic_index,
...
<div class="hero">
    ...
    <p>XGBoost pipeline (Optuna-tuned) trained on METABRIC — 2,000+ real clinical patients.</p>
</div>
""", unsafe_allow_html=True)
...
<div class="stat-row">
    ...
    <div class="stat-pill"><div class="val">XGBoost</div><div class="lbl">Model</div></div>
    <div class="stat-pill"><div class="val">Optuna</div><div class="lbl">Tuning</div></div>
</div>
""", unsafe_allow_html=True)
...
if predict_clicked:
    ...
    with col1:
        st.markdown(f"""<div class="prob-card prob-living">
    ...
    with col2:
        st.markdown(f"""<div class="prob-card prob-deceased">
    ...
    <div class="instruction-card">
        <div class="icon">👈</div>
        <p><strong>Fill in the patient details</strong> in the sidebar and press
        <strong>Predict Survival</strong> to see the result.</p>
    </div>""", unsafe_allow_html=True)
